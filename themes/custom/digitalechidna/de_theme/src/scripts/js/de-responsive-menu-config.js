    ...
    attach: function (context) {
      ...
      $(context).find('body').once('mm-menu').each(function () {
        if (typeof Mmenu !== 'undefined') {
          ...
          // Mobile menu active trail items_block--card-grid.scss
          $('.masthead .block--menu--main .menu-item--active-trail').each(function() {
            var linkTitle = $(this).children('a').children('.link-inner').text();
            $('.mm-listitem__text').filter( function() {
              return $(this).text() === linkTitle;
            }).closest('.mm-listitem').addClass('menu-item--active-trail');
          });
        }
      });
      ...
      if (!$('body').hasClass('user-logged-in')) {
        if ($('#mm-1 .mm-navbar .user-menu--trigger').length < 1) {
          var $mobileLoginLink = $('.user-menu--trigger').clone();
          $mobileLoginLink.prependTo('#mm-1 .mm-navbar');
        }
      } else if ($('#mm-1 .mm-navbar #block-user-menu-mobile').length < 1) {
        var $mobileAccountMenu = $('.user-menu--header .menu--account').clone();
        var $mobileUserImage   = $('.user-menu--header .field--name-user-picture').clone();
        $mobileAccountMenu
          .attr('id', 'block-user-menu-mobile')
          .addClass('menu--account--mobile')
          .removeClass('dropdown-menu')
          .removeClass('dropdown-menu-right')
          .prepend($mobileUserImage);

        $mobileAccountMenu.prependTo('#mm-1 .mm-navbar');
      }
    }
  };
